...
export default function StatsPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (legacy && !classicExists) {
        localStorage.setItem(KEY_CLASSIC, legacy);
        // optional:
      ...
      <header className="flex items-center justify-between mb-8 max-w-6xl mx-auto stats-stagger">
        <div className="flex flex-col gap-2">
          ...
          <div className="flex gap-2">
            ...
            <span className="text-xs text-white/50 self-center ml-2">
              {mode === "competitive" ? "Backspace penalty mode stats" : "Normal mode stats"}
            </span>
          </div>
        </div>
