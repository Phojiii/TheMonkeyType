...
export async function postSiteAnnouncement(announcement) {
  ...
  const lines = [
    `**${announcement.discordTitle || `TheMonkeyType v${announcement.version} Update`}**`,
    announcement.discordIntro || "",
    "",
    "**What's New**",
    ...((announcement.discordSections || []).flatMap((section) => [
      formatSection(section),
      "",
    ])),
    announcement.discordClosing || "",
    `${getSiteUrl()}/guide`,
  ].filter(Boolean);
