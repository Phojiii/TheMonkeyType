...
const CONTACT_CHANNEL_ID =
  process.env.DISCORD_CONTACT_CHANNEL_ID || "1525802223156727878";
...
export async function postContactSubmission({
  ...
  const content = [
    ...
    userId ? `**Signed In User ID:** ${userId}` : null,
    `**Email:** ${email}`,
    `**Category:** ${category}`,
    "",
    "**Message:**",
  ...
  if (!image || typeof image.arrayBuffer !== "function" || !image.size) {
    return postDiscordMessage(CONTACT_CHANNEL_ID, {
      content,
      allowed_mentions: { parse: [] },
    });
  }
  ...
  const blob = new Blob([bytes], { type: image.type || "application/octet-stream" });
  const safeName = String(image.name || "attachment")
    .replace(/[^a-zA-Z0-9._-]/g, "-")
    .slice(0, 120);
  form.append("files[0]", blob, safeName || "attachment");

  const response = await fetch(`https://discord.com/api/v10/channels/${CONTACT_CHANNEL_ID}/messages`, {
    method: "POST",
    headers: {
      Authorization: `Bot ${token}`,
    },
  ...
  return {
    ...
    messageId: String(discordMessage?.id || ""),
  };
}
